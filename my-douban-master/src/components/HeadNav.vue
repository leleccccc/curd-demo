<template>
  <div class="head-nav fix">
    <ul>
      <li class="douban"><router-link style="color: #00b600; font-size: 1.6rem; font-weight: bold;" to="/homeview">豆瓣</router-link></li>
      <li><router-link style="color: #2384E8;" to="/movieview">电影</router-link></li>
      <li><router-link style="color: #9F7860;" to="/bookview">图书</router-link></li>
      <li><router-link style="color: #E4A813;" to="/broadcastview">广播</router-link></li>
      <li><router-link style="color: #2AB8CC;" to="/groupview">小组</router-link></li>
      <li><router-link to="/rearchview"></router-link></li>
    </ul>
  </div>
</template>

<script>
export default {
  name: 'HeadNav',
  data () {
    return {
    }
  }
}
</script>

<style lang="scss" scoped>
.head-nav {
  top: 0;
  left: 0;
  width: 100%;
  height: 3rem;
  border-bottom: 0.1rem solid #f3f3f3;
  z-index: 999;
}

ul {
  display: flex;
  flex: 1;
  justify-content: flex-end;
  width: 100%;
  height: 100%;
  li{
    width: 15%;
    height: 100%;
    text-align: center;
    line-height: 3rem;
  &:last-child{
    font-size: 0;
    background: url(data:image/png;base64,) no-repeat;
    background-size: 50% 50%;
    background-position: center;
  }
  }
}

.douban {
  flex:1;
  justify-content: flex-start;
}
</style>
